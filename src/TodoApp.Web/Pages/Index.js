$(function () {
    ...
    $('#TodoList').on('click', 'li i', function () {
        ...
        todoApp.todo.getById(id).then((item) => {
            console.log("Funcionou:" + item.id)
        })
        ...
        if (todoText) {
            todoApp.todo.create(todoText).then(function (result) {
                $('<li data-id="' + result.id + '">')
                    ...
                    .appendTo($('#TodoList'));
                $('#NewItemText').val('');
                abp.notify.success('Item incluso com sucesso');
            });
        } else {
            abp.notify.info('Campo texto não pode ser vazio');
        }
